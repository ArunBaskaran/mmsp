// mmsp2txt.cpp
// Convert MMSP grid data to comma-delimited ASCII format (TXT)
...
template <int dim, typename T> void convert_scalars(const MMSP::grid<dim,T>& GRID, std::ofstream& txtfil);

template <int dim, typename T> void convert_vectors(const MMSP::grid<dim,MMSP::vector<T> >& GRID, std::ofstream& txtfil);

template <int dim, typename T> void convert_sparses(const MMSP::grid<dim,MMSP::sparse<T> >& GRID, std::ofstream& txtfil);
...
int main(int argc, char* argv[])
{
	...
	if (std::string(argv[1]) == "--help") {
		std::cout << argv[0] << ": convert MMSP grid data to comma-delimited ASCII (txt) format.\n";
	...
	if (argc==2) {
		...
		if (datname.find_first_of("0123456789",extpos) != std::string::npos)
			...
			txtname << datname.substr(0, extpos) << ".txt";
	} else
		txtname << argv[2];

	// Open output file
	std::ofstream txtfil(txtname.str().c_str());
...
template <int dim, typename T> void convert_scalars(const MMSP::grid<dim,T>& GRID, std::ofstream& txtfil)
{
	if (dim==1) {
		for (int n=0; n<MMSP::nodes(GRID); n++) {
			...
			txtfil << dx(GRID,0)*x[0];
			for (int d=1; d<dim; d++)
				txtfil << ',' << dx(GRID,d)*x[d];
			txtfil << GRID(n) << '\n';
		}
	} else if (dim==2) {
		for (int n=0; n<MMSP::nodes(GRID); n++) {
	...
	if (dim==1) {
		for (int n=0; n<MMSP::nodes(GRID); n++) {
			...
			for (int h=0; h<length(GRID(n)); h++)
				txtfil << ',' << GRID(n).index(h) << ',' << GRID(n).value(h);
			txtfil << '\n';
		}
	} else if (dim==2) {
		for (int n=0; n<MMSP::nodes(GRID); n++) {
